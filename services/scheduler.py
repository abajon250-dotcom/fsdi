...
async def check_pending_payments():
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute(
            "SELECT id, user_id, chat_id, type, data, amount, currency, provider, payment_id, duration "
            "FROM pending_payments WHERE status = 'pending' AND provider = 'cryptobot'"
        ) as cursor:
            pendings = await cursor.fetchall()

    for (pending_id, user_id, chat_id, sub_type, data, amount, currency, provider, payment_id, duration) in pendings:
